...
double** read_file(unsigned int &rows, unsigned int &columns, string s) {
	double** matrix = nullptr;
	char op;
	ifstream fin;
	fin.open(s.c_str());
	if (!fin.is_open()) {
		std::cerr << "file ope nfail" << endl;
		return nullptr;
	}
	string line;
	getline(fin, line);
...
int main()
{
	...
	matrix1 = read_file(str1, col1, name1);// mat1 = true;
	...
	if (op == '+' || op == '-' || op == '*')
	{
		...
		matrix2 = read_file(str2, col2, name2); //mat2 = true;
		switch (op)
		{
		case '+':
